...
public class UIInteractionHandler {
    ...
    public static void onRightClickItem(PlayerInteractEvent.RightClickItem event) {
        ...
        if (uiId != null) {
            // 打开UI逻辑
            openUI(player, uiId, "item", itemId.toString());
    ...
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (uiId != null) {
            // 打开UI逻辑
            openUI(player, uiId, "block", blockId.toString());
            event.setCanceled(true);
            LOGGER.debug("Opened UI {} for block {} clicked by player {}", uiId, blockId, player.getName().getString());
        }
    }
    
    private static void openUI(Player player, ResourceLocation uiId, String type, String targetId) {
        if (!player.level().isClientSide) {
            ...
            if (uiNode != null) {
                ...
                if (player instanceof ServerPlayer serverPlayer) {
                    openUIOnClient(serverPlayer, uiId, uiNode);
                }
            }
        }
    }

    private static void openUIOnClient(ServerPlayer player, ResourceLocation uiId, MCMLNode uiNode) {
        try {
            // 检查网络通道是否已注册
            ...
            OpenUIPacket packet = new OpenUIPacket(uiId);
